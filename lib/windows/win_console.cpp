...
namespace Is
{
		...
		, m_divideY(divideY)
	{
		...
		// ANSI エスケープシーケンスを有効化
		enable_ansi_escapesequence(); // デフォルトで有効になっている
	 ...
	 * \return 
	 */
	Win32ConsoleStatus Win32Console::get_mode()
	{
		if (m_hWnd != NULL)
		{
			// コンソール入力バッファのモードを取得
			...
			{
				// 失敗
				show_win32api_error(m_hWnd, CString("入力バッファモードの取得失敗")); // Win32 API エラーメッセージを取得
			...
			{
				// 失敗
				show_win32api_error(m_hWnd, CString("スクリーンバッファモードの取得失敗")); // Win32 API エラーメッセージを取得
	 ...
	 * \return 
	 */
	Win32ConsoleStatus Win32Console::enable_ansi_escapesequence()
	{
		if (m_hWnd != NULL)
		{
			/**
			...
			// 		show_win32api_error(m_hWnd, CString("入力バッファモードのANSI ESCAPEの有効化失敗")); // Win32 API エラーメッセージを取得
			...
			// {
			// 	// 標準入力につながっていない
			// 	show_win32api_error(m_hWnd, CString("標準入力に未接続")); // Win32 API エラーメッセージを取得
			...
			if (_isatty(m_fdOut))
			{
				...
				else
				{
					...
					show_win32api_error(m_hWnd, CString("スクリーンバッファモードのANSI ESCAPEの有効化失敗")); // Win32 API エラーメッセージを取得
			...
			{
				// 標準出力につながっていない
				show_win32api_error(m_hWnd, CString("標準出力に未接続")); // Win32 API エラーメッセージを取得
